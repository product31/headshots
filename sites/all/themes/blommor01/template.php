<?php
// $Id: template.php,v 1.2.2.1 2009/04/21 07:55:10 frjo Exp $
 ...
 */
function blommor01_preprocess_page(&$vars, $hook) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = blommor01_id_safe('page-'. $path);
    $body_classes[] = blommor01_id_safe('section-'. $section);
    if (arg(0) == 'node') {
      if (arg(1) == 'add') {
        if ($section == 'node') {
 ...
 */
function blommor01_preprocess_node(&$vars, $hook) {
  global $user;
 ...
 */
function blommor01_preprocess_comment(&$vars, $hook) {
  global $user;
 ...
 */
function blommor01_node_submitted($node) {
...
function blommor01_comment_submitted($comment) {
  return t('!user - <abbr class="created" title="!microdate">!date</abbr>', array(
 ...
 */
function blommor01_comment_wrapper($content, $type = NULL) {
  static $node_type;
 ...
 * @ingroup themeable
 */
function blommor01_menu_local_tasks() {
  $output = '';
 ...
 */
function blommor01_id_safe($string) {
